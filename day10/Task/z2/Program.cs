try
{
    ...
    Train train = new Train("Москва", "a1", new DateTime(2011, 5, 26, 20, 0, 0, 0));
    Train train1 = new Train("Киев", "a2", new DateTime(2011, 5, 26, 20, 10, 0, 0));
    Train train2 = new Train("Минск", "a3", new DateTime(2011, 5, 26, 20, 20, 0, 0));
    Train train3 = new Train("Париж", "a4", new DateTime(2011, 5, 26, 22, 0, 0, 0));
...
class Station
{
    ...
    public void GetInformation(int index) // получение информации по индексу
    {
        //var isMore = trains[0] > trains[1]; // проверка перегрузки
    ...
    public void ThisDate(DateTime time) // получение данных о поездах, которые поедут после заданного времение
    {
    ...
    public void TripFromPointName(string PointName) // получение поездов, которые направляются в определенную точку
    {
